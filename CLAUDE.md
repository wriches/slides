...
A collection of HTML slide deck presentations hosted on GitHub Pages at `slides.riches.im`. Each presentation is a self-contained single HTML file with embedded CSS, JS, and assets.

## Repository Structure

- **Root `index.html`** — Dynamic landing page that fetches directory listings from the GitHub API (`wriches/slides`) and renders links to all presentations. No build step required; adding a new presentation directory and pushing is sufficient.
...
Copy an existing presentation directory, rename it with the new date and name, and update date/event references inside `index.html`. The landing page discovers new directories automatically via the GitHub API.
...
- **Navigation**: Arrow keys / Space for next, ArrowLeft for prev, `f` for fullscreen. Click buttons also provided.
...
- **Slide comments**: Each slide is preceded by `<!-- SLIDE N: DESCRIPTION -->` comments with sequential numbering. Keep these updated when adding, removing, or reordering slides.
